...
function ProjectCard({ img, title, description, date_start, date_end=null, array, link, available }) {
    return (
        <div className="flex flex-col m-5 bg-white w-80 h-100 rounded-md mb-10 cursor-pointer">
            <div className="rounded-lg w-full">
                <img className="rounded-tl-md rounded-tr-md w-full h-40 object-cover" alt="Photo du projet" src={img} />
            </div>
            <div className="rounded-lg w-100">
                ...
                <div className="flex p-2 pl-4 text-[#757483]">
                    <span className="flex flex-row text-sm">
                        ...
                        {date_end && (
                            <>
                                &nbsp;
                                <LeftArrowIcon size="5" strokeWidth="1.5" />
                                &nbsp; {date_end}
                            </>
                        )}
                    </span>
                ...
                <div className="h-20 flex p-2 pl-4 py-4 place-content-center text-white font-semibold text-lg">
                    <Link to={link} className={`flex flex-row rounded-md p-2 ${!available ? "bg-[#13151c] elay-50 hover:bg-[#222222] text-[#f9f9f9]" : "bg-red-700 text-white cursor-not-allowed"} `}> {!available ? "En savoir plus" : "⛔ Indisponible"}</Link>
                </div>
            </div>
        </div>
    );
